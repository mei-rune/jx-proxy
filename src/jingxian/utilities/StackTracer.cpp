...
{
public:
    StackWalkerInternal(StackTracer *parent, HANDLE hProcess)
    {
        m_parent = parent;
        m_hDbhHelp = NULL;
        pSC = NULL;
...
cleanup:
    ...
    DWORD LoadModule(HANDLE hProcess, LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size)
    {
        ...
        if ((m_parent != NULL) && (szImg != NULL))
        {
            ...
            if ((this->m_parent->m_options & StackTracer::RetrieveFileVersion) != 0)
            {
...
// #############################################################
StackTracer::StackTracer(DWORD dwProcessId, HANDLE hProcess)
{
...
StackTracer::StackTracer(int options, LPCSTR szSymPath, DWORD dwProcessId, HANDLE hProcess)
{
...
static StackTracer::PReadProcessMemoryRoutine s_readMemoryFunction = NULL;
    ...
    for (frameNum = 0; ; ++frameNum)
    {
        ...
        if (s.AddrPC.Offset != 0 && frameNum >= skipFramesCount)
        {
            ...
            { // yes, we have SymGetLineFromAddr64()
                ...
                {
                    if ((m_options & StackTracer::RetrieveGeneralInfo) != 0)
                    {
...
BOOL __stdcall StackTracer::myReadProcMem(
    HANDLE      hProcess,
    DWORD64     qwBaseAddress,
    PVOID       lpBuffer,
    DWORD       nSize,
...
void StackTracer::OnLoadModule(LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCSTR symType, LPCSTR pdbName, ULONGLONG fileVersion)
{
    if ((m_options & StackTracer::RetrieveGeneralInfo) != 0)
    {
...
void StackTracer::OnSymInit(LPCSTR szSearchPath, DWORD symOptions, LPCSTR szUserName)
{
    if ((m_options & StackTracer::RetrieveGeneralInfo) != 0)
    {
        CHAR buffer[STACKWALK_MAX_NAMELEN];
